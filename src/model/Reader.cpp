...
void Reader::parseInputFile(char* fileName)
{
	...
	for (unsigned int i = 0; i < num; i++)
	{
		...
		for (unsigned int i = 0; i < BUFFER.length(); i++)// contains '\t' in string?
			if (BUFFER[i] == '\t')
			{
				tab = true;
				break;
		...
		if (type == TypeAndGate || type == TypeNandGate || type == TypeNotGate
				|| type == TypeOrGate || type == TypeNorGate)
	...
	while (true)
	{
		...
		if (BUFFER.length() - sidO.length() - spinO.length() - sidI.length()
				- spinI.length() - 3 > 0)
			flag = true;
		...
		if (sidO.empty() || sidI.empty() || spinO.empty() || spinI.empty()
				|| pinX > 3 || pinY > 3 || pinX < 0 || pinY < 0 || flag)
			break;

		ConnectionDescription* cd = new ConnectionDescription(idX, pinX, idY,
				pinY);
...
void Reader::sourceInitialize(unsigned int id, string descr)
{
	...
	if (type == TypeImpulsSource)
	{
		...
		for (unsigned int i = 0; i < data->getElements().size(); i++)
		{
			if (data->getElements()[i]->getId() == id)
			{
				ISource* s = new ISource();			// TODO: constructors?
